  ...
  return [
    {
      ...
      create() {
        ...
        cancelButton.className =
          "font-sans p-1 text-muted-foreground hover:text-foreground hover:bg-foreground/10 rounded-sm";
...
const quickEditTooltipField = StateField.define<readonly Tooltip[]>({
  ...
  update(tooltips, transaction) {
    if (transaction.effects.some((effect) => effect.is(showQuickEditEffect))) {
